...
class MeanTeacherPipeline:
    ...
    def __init__(self, filenames, batch_size, image_size,
        ...
        padded_shapes = ([2, self.image_height, self.image_width, 3], [2, None, 4], [], [])
    ...
    def _augmentation_fn(self, image, boxes):
        ...
        # source augmentation
        s_image = random_color_manipulations(image, probability=0.45, grayscale_probability=0.05)
        s_image = random_pixel_value_scale(s_image, minval=0.85, maxval=1.15, probability=0.2)
        s_boxes = random_jitter_boxes(boxes, ratio=0.01)

        # teacher augmentation
        t_image = random_color_manipulations(image, probability=0.45, grayscale_probability=0.05)
        t_image = random_pixel_value_scale(t_image, minval=0.85, maxval=1.15, probability=0.2)
        t_boxes = random_jitter_boxes(boxes, ratio=0.01)

        set_images = [s_image, t_image]
        set_boxes = [s_boxes, t_boxes]
        return set_images, set_boxes
