...
from src.constants import MATCHING_THRESHOLD, PARALLEL_ITERATIONS
...
class Detector:
    ...
    def get_class_prediction(self):
        """
        return class_predictions_with_background with shape (batch_size, num_anchors)
        """
        return tf.nn.softmax(self.class_predictions_with_background, axis=2)[:, :, 1]
    ...
    def _add_box_predictions(self, feature_maps):
        ...
        with tf.name_scope('reshaping'):
            for i in range(num_feature_maps):
                ...
                class_predictions_with_background[i] = tf.reshape(y,
                                                                  tf.stack([batch_size, num_anchors_on_feature_map, 2]))
