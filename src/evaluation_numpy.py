...
class EvaluatorNumpy:
    ...
    def get_metric(self, image_name, groundtruth, predictions):
        """
        Arguments:
            ...
            predictions: a dict with the following keys
                ...
                'num_boxes': an int tensor with shape [1].
        """
        boxes = np.array(groundtruth['boxes'][0])
        num_boxes = np.array(groundtruth['num_boxes'][0])

        self.add_groundtruth(image_name[0], boxes, num_boxes)
        self.add_detections(image_name[0], predictions['boxes'][0],
                            predictions['scores'][0], predictions['num_boxes'][0])

        self.metrics = evaluate_detector(
            self.groundtruth_by_image,
            self.detections, self.iou_threshold
        )

        metric_names = ['AP', 'precision', 'recall', 'mean_iou', 'threshold', 'FP', 'FN']
        eval_metric_ops = {
            measure: np.float32(self.metrics[measure]) for measure in metric_names
        }
